...
Every skill ships its code on `main` — nothing to merge, nothing to patch. Run
`/<name>` to use one: **feature** skills (Telegram, voice, calendar, memory, …)
switch on code that is already present by doing the auth, registration and
config work that needs your credentials; **operational** skills are
instruction-only workflows; **utility** skills carry helper code alongside their
`SKILL.md`.

A clone therefore contains every feature, each inert until something you provide
switches it on — a token in `.env`, a mounted credential, a port. Local install
state (`.env`, `store/`, `groups/<name>/`) stays out of git: that is data and
secrets, not code.
