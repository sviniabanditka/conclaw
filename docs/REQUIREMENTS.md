...
ConClaw gives you the core functionality without that mess.
...
- Trigger: `@ConClaw` prefix (case insensitive), configurable via `ASSISTANT_NAME` env var
...
- Custom `conclaw` MCP server (inside container) provides scheduling tools
...
- `/update-conclaw` - Pull upstream changes, merge with customizations
...
- **Trigger**: `@ConClaw` (case insensitive)
- **Response prefix**: `ConClaw:`
...
**ConClaw** - A reference to Clawdbot (now OpenClaw).
