...
│  │    • mcp__conclaw__* (scheduler tools via IPC)                │    │
│  │                                                                │    │
│  └──────────────────────────────────────────────────────────────┘    │
│                                                                       │
└───────────────────────────────────────────────────────────────────────┘
...
│   ├── conclaw.log               # Host stdout
│   └── conclaw.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.conclaw.plist         # macOS service configuration
```
...
export const ASSISTANT_NAME = process.env.ASSISTANT_NAME || 'ConClaw';
...
export const CONTAINER_IMAGE = process.env.CONTAINER_IMAGE || 'conclaw-agent:latest';
...
- `{{PROJECT_ROOT}}` - Absolute path to your conclaw installation
...
ConClaw uses a hierarchical memory system based on CLAUDE.md files.
...
Messages must start with the trigger pattern (default: `@ConClaw`):
- `@ConClaw what's the weather?` → ✅ Triggers Claude
- `@andy help me` → ✅ Triggers (case insensitive)
- `Hey @ConClaw` → ❌ Ignored (trigger not at start)
...
[Jan 31 2:35 PM] John: @ConClaw what toppings do you recommend?
```
...
|---------|---------|--------|
| `@Assistant [message]` | `@ConClaw what's the weather?` | Talk to Claude |

### Commands Available in Main Channel Only

| Command | Example | Effect |
|---------|---------|--------|
| `@Assistant add group "Name"` | `@ConClaw add group "Family Chat"` | Register a new group |
| `@Assistant remove group "Name"` | `@ConClaw remove group "Work Team"` | Unregister a group |
| `@Assistant list groups` | `@ConClaw list groups` | Show registered groups |
| `@Assistant remember [fact]` | `@ConClaw remember I prefer dark mode` | Add to global memory |

---

## Scheduled Tasks

ConClaw has a built-in scheduler that runs tasks as full agents in their group's context.
...
```
User: @ConClaw remind me every Monday at 9am to review the weekly metrics

Claude: [calls mcp__conclaw__schedule_task]
        {
...
```
User: @ConClaw at 5pm today, send me a summary of today's emails
...
From any group:
- `@ConClaw list my scheduled tasks` - View tasks for this group
- `@ConClaw pause task [id]` - Pause a task
- `@ConClaw resume task [id]` - Resume a paused task
- `@ConClaw cancel task [id]` - Delete a task

From main channel:
- `@ConClaw list all tasks` - View tasks from all groups
- `@ConClaw schedule task for "Family Chat": [prompt]` - Schedule for another group

---

## MCP Servers

### ConClaw MCP (built-in)

The `conclaw` MCP server is created dynamically per agent call with the current group's context.
...
ConClaw runs as a single macOS launchd service.

### Startup Sequence

When ConClaw starts, it:
1. **Ensures container runtime is running** - Automatically starts it if needed; kills orphaned ConClaw containers from previous runs
...
<dict>
    ...
    <dict>
        ...
        <string>ConClaw</string>
    </dict>
    <key>StandardOutPath</key>
    <string>{{PROJECT_ROOT}}/logs/conclaw.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/conclaw.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.conclaw.plist ~/Library/LaunchAgents/
...
|-------|-------|----------|
| No response to messages | Service not running | Check `launchctl list | grep conclaw` |
| "Claude Code process exited with code 1" | Container runtime failed to start | Check logs; ConClaw auto-starts container runtime but may fail |
| "Claude Code process exited with code 1" | Session mount path wrong | Ensure mount is to `/home/node/.claude/` not `/root/.claude/` |
| Session not continuing | Session ID not saved | Check SQLite: `sqlite3 store/messages.db "SELECT * FROM sessions"` |
| Session not continuing | Mount path mismatch | Container user is `node` with HOME=/home/node; sessions must be at `/home/node/.claude/` |
| "QR code expired" | WhatsApp session expired | Delete store/auth/ and restart |
| "No groups registered" | Haven't added groups | Use `@ConClaw add group "Name"` in main |

### Log Location

- `logs/conclaw.log` - stdout
- `logs/conclaw.error.log` - stderr
