...
import { STORE_DIR, TEMPLATE_ASSISTANT_NAME } from '../src/config.ts';
import { renameAssistant } from '../src/assistant-name.ts';
...
function parseArgs(args: string[]): RegisterArgs {
  const result: RegisterArgs = {
    ...
    isMain: false,
    assistantName: TEMPLATE_ASSISTANT_NAME,
  };
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case '--is-main':
        ...
        break;
      case '--assistant-name':
        result.assistantName = args[++i] || TEMPLATE_ASSISTANT_NAME;
        break;
    }
  }
...
export async function run(args: string[]): Promise<void> {
  ...
  if (parsed.assistantName !== TEMPLATE_ASSISTANT_NAME) {
    logger.info(
      { from: TEMPLATE_ASSISTANT_NAME, to: parsed.assistantName },
