...
<dict>
  ...
  try {
    ...
    serviceLoaded = output.includes('com.conclaw');
  } catch {
...
/**
 * Kill any orphaned conclaw node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned conclaw processes');
  } catch {
...
function setupSystemd(
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/conclaw.service';
    ...
    unitPath = path.join(unitDir, 'conclaw.service');
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned conclaw processes to avoid channel connection conflicts
  ...
  try {
    execSync(`${systemctlPrefix} enable conclaw`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start conclaw`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active conclaw`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, 'start-conclaw.sh');
  const pidFile = path.join(projectRoot, 'conclaw.pid');

  const lines = [
    '#!/bin/bash',
    '# start-conclaw.sh — Start ConClaw without systemd',
    ...
    '    echo "Stopping existing ConClaw (PID $OLD_PID)..."',
    '    kill "$OLD_PID" 2>/dev/null || true',
    '    sleep 2',
    '  fi',
    'fi',
    '',
    'echo "Starting ConClaw..."',
    `nohup ${JSON.stringify(nodePath)} ${JSON.stringify(projectRoot + '/dist/index.js')} \\`,
    `  >> ${JSON.stringify(projectRoot + '/logs/conclaw.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/conclaw.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "ConClaw started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/conclaw.log"`,
  ];
