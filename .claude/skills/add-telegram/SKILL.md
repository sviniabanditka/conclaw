---
name: add-telegram
description: Add Telegram as a channel. Can be the primary channel or run alongside others. Also configurable as a control-only channel (triggers actions) or passive channel (receives notifications only).
---

# Add Telegram Channel

This skill adds Telegram support to ConClaw, then walks through interactive setup.
...
>    - Bot name: Something friendly (e.g., "ConClaw Assistant")
...
> - For non-main: `@ConClaw hello` or @mention the bot
>
...
4. Service is running: `launchctl list | grep conclaw` (macOS) or `systemctl --user status conclaw` (Linux)
...
- Check bot is started: `tail -f logs/conclaw.log`
...
6. Rebuild: `npm run build && launchctl kickstart -k gui/$(id -u)/com.conclaw` (macOS) or `npm run build && systemctl --user restart conclaw` (Linux)
