---
name: claw
description: Install the claw CLI tool — run ConClaw agent containers from the command line without opening a chat app.
---

# claw — ConClaw CLI

`claw` is a Python CLI that sends prompts directly to a ConClaw agent container from the terminal. It reads registered groups from the ConClaw database, picks up secrets from `.env`, and pipes a JSON payload into a container run — no chat app required.
...
Run this skill from within the ConClaw directory. The script auto-detects its location, so the symlink always points to the right place.
...
You should see registered groups. If ConClaw isn't running or the database doesn't exist yet, the list will be empty — that's fine.
...
claw --image conclaw-agent:dev "Hello"
...
The script auto-detects your ConClaw directory and reads `.env` from it. Check that the file exists and contains at least one of: `CLAUDE_CODE_OAUTH_TOKEN`, `ANTHROPIC_API_KEY`, `ANTHROPIC_AUTH_TOKEN`.

### Container times out

The default timeout is 300 seconds. For longer tasks, pass `--timeout 600` (or higher). If the container consistently hangs, check that your `conclaw-agent:latest` image is up to date by running `./container/build.sh`.
...
### Override the ConClaw directory

If `claw` can't find your database or `.env`, set the `CONCLAW_DIR` environment variable:

```bash
export CONCLAW_DIR=/path/to/your/conclaw
```
