...
|-----|----------|---------|
| **Main app logs** | `logs/conclaw.log` | Host-side routing, container spawning |
| **Main app errors** | `logs/conclaw.error.log` | Host-side errors |
...
echo '{}' | docker run -i \
  ...
  --entrypoint /bin/bash conclaw-agent:latest \
...
```bash
docker run --rm --entrypoint /bin/bash conclaw-agent:latest -c 'ls -la /workspace/'
```
...
```bash
docker run --rm --entrypoint /bin/bash conclaw-agent:latest -c '
  whoami
  ls -la /workspace/
  ls -la /app/
'
...
# Also clear the session ID from ConClaw's tracking (stored in SQLite)
...
```bash
echo "=== Checking ConClaw Container Setup ==="
...
echo '{}' | docker run -i --entrypoint /bin/echo conclaw-agent:latest "OK" 2>/dev/null || echo "MISSING - run ./container/build.sh"
